...
def distributions_page():
    ...
    if page == "Uniforme Discreta":
        ...
        st.write("\n")
        st.write("\n")
        n = st.number_input("Insira uma quantidade amostral entre 0 e 100:",min_value=1, max_value=100, key = 2)
        fig = udisc(n)
        st.plotly_chart(fig, use_container_width=True)
    ...
    if page == "Bernoulli":
        ...
        st.write("\n")
        st.write("\n")
        teta = st.slider("Insira uma probabilidade entre 0 e 1:", 0.0, 1.0)
        fig = bern_dist(teta)
        st.plotly_chart(fig, use_container_width=True)
    if page == "Binomial":
        st.title("Distribuições")
        st.write("\n")
        st.write("\n")
        n = st.slider("Insira uma quantidade amostral entre 0 e 100:",min_value=1, max_value=100, key = 3)
        teta = st.slider("Insira uma probabilidade entre 0 e 1:", 0.0, 1.0)
        fig = binom(teta, n)
        st.plotly_chart(fig, use_container_width=True)
        accumulation = st.button("Função Acumulada de Probabilidade")
    ...
    if page == "Hipergeométrica":
        ...
        st.write("\n")
        st.write("\n")
        N = st.number_input("Insira uma quantidade amostral entre 0 e 100:",min_value=1, max_value=100, key = 3)
    ...
    if page =="Poisson":
        ...
        st.write("\n")
        st.write("\n")
        N = st.number_input("Insira uma quantidade amostral entre 0 e 100:",min_value=1, max_value=100, key = 5)
        p = st.slider("Insira uma probabilidade de 0 a 1", 0.0, 1.0)
        fig = dpoisson(N, p)
        st.plotly_chart(fig, use_container_width=True)
    if page =="Binomial Negativa":
        ...
        st.write("\n")
        st.write("\n")
        x = st.slider("Insira uma quantidade amostral entre 0 e 100:", 0, 100, key = 100)
        r = st.number_input("Insira um r entre 0 e 20:", 0, 20, key = 101)
        teta = st.slider("Insira uma probabilidade entre 0 e 1:", 0.0, 1.0, key = 102)
        fig = bndist(r, x, teta)
        st.plotly_chart(fig, use_container_width=True)
        accumulation = st.button("Função Acumulada de Probabilidade")
        if accumulation:
